...
Public Class payroll
    ...
    Private Sub payroll_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            sql = "SELECT * from user where appointment like '%" & "Contractual" & "%'   "
            ...
            Do While DR.Read = True
                ...
                adp1.SelectCommand = New MySqlCommand("SELECT (rate)  from admin ", sql_connection1)
                ...
                DataGridView1.Rows.Add(DR("idnum"), DR("lname"), DR("fname"), DR("mname"), DR("appointment"), tbl.Rows(0)("SEC_TO_TIME(SUM(TIME_TO_SEC(alltotal)))").ToString, tbl1.Rows(0)("rate").ToString, tbl.Rows(0)("sum(gp)").ToString, tbl.Rows(0)("sum(madeduct)").ToString, tbl.Rows(0)("sum(debit)").ToString)
    ...
    '    sqla = "delete from payroll"
    '    sql_command = New MySqlClient.MySqlCommand(sqla, sql_connection)
    ...
    '    '    'dapat may query dito kung anong date 
    ...
    '    '        'sql_command1 = New MySqlClient.MySqlCommand("SELECT sum(alltotal) from dtr where idnum = '" & DR("idnum") & "' ", sql_connection1)
    '    '        'adp.SelectCommand = New MySqlCommand("SELECT sum(alltotal) from dtr where idnum = '" & DR("idnum") & "' ", sql_connection1)
    '    '        adp.SelectCommand = New MySqlCommand("SELECT * from dtr where idnum = '" & DR("idnum") & "' ", sql_connection1)
    ...
    '    '        'DataGridView1.Rows.Add(DR("idnum"), DR("lname"), DR("fname"), DR("appointment"), DR("allhours"), DR("rate"), DR("amount"), DR("deduction"))
    '    '        'DataGridView1.Rows.Add(AA, DR("lname"), DR("fname"), DR("appointment"), DR1("alltotal"), DR("rate"), DR("amount"), DR("deduction"))
    '    '        DataGridView1.Rows.Add(DR("idnum"), DR("lname"), DR("fname"), tbl.Rows(0)("alltotal").ToString) 'tbl.rows(row)(column) '0=1st  row '1st lang e didisplay kahit maraming nakuha wala kasing specific date select all record gnamit mo
    ...
    Private Sub IndividualRecordsToolStripMenuItem1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        dtr.Show()
        Me.Close()
